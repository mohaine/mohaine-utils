...
public class ReflectionLoadFieldDefinition extends AbstractLoadFieldDefinition {
    ...
    public ReflectionLoadFieldDefinition(String columnName, Field field, DbType dbType) {
        super(columnName);
        this.field = field;
    ...
    static Object getValueToSet(Field field, DbType type, Object value) {
        if (DbType.Integer.equals(type)) {
            return value != null ? new Integer(((Number) value).intValue()) : null;
        } else if (DbType.Double.equals(type)) {
            return value != null ? new Double(((Number) value).doubleValue()) : null;
        } else if (DbType.Long.equals(type)) {
            return value != null ? new Long(((Number) value).longValue()) : null;
