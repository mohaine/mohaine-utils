...
public class PrintableDriver implements Driver {
    ...
    static {
        try {
            Enumeration driversEnum = DriverManager.getDrivers();
     ...
     */
    public Connection connect(String connectString, Properties props) throws SQLException {
    ...
    protected PrintableDriverController createControl() {
        ...
        if (controlClass != null && controlClass.length() > 0) {
            try {
                return (PrintableDriverController) Class.forName(controlClass).newInstance();
            } catch (Exception e) {
                throw new RuntimeException("Failed to create controlClass " + controlClass, e);
            }
        }

        return new PrintableDriverController() {
            @Override
            public SqlPrinter getPrinterFor(String connectString, Connection conn) {
                return new SqlPrinterConsole();
            }

            @Override
            public String modifyConnection(String connectString, Properties props) {
